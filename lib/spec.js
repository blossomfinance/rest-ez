...
import RestEZRequest from './request';
import fs from 'fs';
...
export default class Spec {
    ...
    async processRequest(opts) {
        let _req = new RestEZRequest(opts, this);
        return _req.send();
    }
    ...
    async runHook(type, hookData, opts) {
        try {
            ...
            if (data.run_type === 'inline') {
                if (data.inline) {
                    const inlineFunction = data.inline.function;
                    await runInlineFunction(inlineFunction, _context, data.inline.args);
                } else {
                ...
                if (data.module) {
                    ...
                    const functionName = data.module.function_name;
                    await runModuleFunction(customModule, functionName, _context, data.module.args);
                } else {
